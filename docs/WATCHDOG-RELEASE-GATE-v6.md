...
The isolated exact-source tests passed: **56/56 selected Watchdog Node** (policy 16, network 7, scheduler 5, signed snapshot crypto 7, snapshot serializer 10, Access JWT/JWKS 8, publication rights guard 3); **25/25 metadata Worker Node** (full 20 catalog/IMDb/cache regressions plus 5 byte-budget tests); **19/19 Python .cs3 staging**; **68/68 Kotlin rights policy**, **19/19 Kotlin Ed25519 snapshot trust**, and **12/12 Kotlin official TMDb collection chronology**. Total **199/199 selected exact-source cases**. JVM tests use real BouncyCastle 1.80; all external HTTP calls in Worker tests are mocks and all ZIPs are synthetic fixtures.

Production signed publication now requires an exact approved D1 registry identity, approved media kind and adapter version, canonical internal rights evidence, and an explicit separate production rights allowlist. The tracked list remains **EMPTY**; test-only fixture approval is injected only into offline factory tests. Admin JWKS responses now have a real 65,536-byte streaming budget and reject malformed UTF-8. Stage-release also rejects symlinked Gradle inputs, not just symlinked output files. No production rights or keys were approved or added.
