...
**Isolated verification:** Exact SHA-matched GitHub `policy.mjs` + its committed test passed **18/18**; exact `publication-guard.mjs` + its committed test passed **8/8**. Additional local isolated tests used exact SHA-matched policy, publication guard, runner and Worker source with clearly marked mock dependencies: **55/55** (14 policy, 21 grant, 10 runner, 10 Worker). Total Node **81/81**. The exact committed 0001/0002 SQL migration files passed **8/8** local SQLite crash/lease/CAS/audit regression scenarios using a small Python harness that mirrors the write SQL. These 89 tests do **not** replace the real full Node test suite or actual Worker+D1 integration. The updated committed `runner.test.mjs` and `signed-endpoint.test.mjs` have not yet run with all their exact GitHub dependencies.
