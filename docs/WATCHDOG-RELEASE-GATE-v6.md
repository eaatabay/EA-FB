...
## 26 Sep 2026: first 30-minute parkur — exact blob verification

This session caught and fixed two previously missed live-code bugs in the v6 feature branch: the Watchdog source policy's overescaped regex rejected normal dotted HTTPS hostnames, and the separate metadata Worker's overescaped `Content-Length` regex skipped the early size rejection. Exact current source and test blobs were transferred to an isolated Node 22 working directory and their local Git blob SHA-1s were compared with the corresponding GitHub blob SHA-1s, rather than merely reconstructing similar tests.

The isolated exact-source tests passed: **57/57 selected Watchdog Node** (policy 16, network 7, scheduler 5, signed snapshot crypto 7, snapshot serializer 10, Access JWT/JWKS 8, publication rights guard 4); **25/25 metadata Worker Node** (full 20 catalog/IMDb/cache regressions plus 5 byte-budget tests); **19/19 Python .cs3 staging**; **68/68 Kotlin rights policy**, **19/19 Kotlin Ed25519 snapshot trust**, and **12/12 Kotlin official TMDb collection chronology**. Total **200/200 selected exact-source cases**. JVM tests use real BouncyCastle 1.80; all external HTTP calls in Worker tests are mocks and all ZIPs are synthetic fixtures.

Production signed publication now binds evidence to the exact rights/YYYY/<source-id>.md document: another source cannot borrow an allowlisted rights record. Production signed publication also requires an exact approved D1 registry identity, approved media kind and adapter version, canonical internal rights evidence, and an explicit separate production rights allowlist. The tracked list remains **EMPTY**; test-only fixture approval is injected only into offline factory tests. Admin JWKS responses now have a real 65,536-byte streaming budget and reject malformed UTF-8. Stage-release also rejects symlinked Gradle inputs, not just symlinked output files. No production rights or keys were approved or added.

**Still blocked:** a complete GitHub checkout `npm test` including SQLite registry, lease, runner, full-cycle and signed-endpoint integration tests; actual Wrangler/workerd with local D1; actual Gradle/Android `.cs3` packaging; and physical Mi Box testing. The local environment has no direct GitHub network, Wrangler or Gradle/Android SDK. No GitHub Actions, deploy, real source probe, remote D1 or production branch modification occurred.

## 26 Sep 2026: second 30-minute offline parkur

**Fixed an actual source-health accounting defect.** Previously an adapter exception, timeout or null result returned `runner_error` without a D1 probe-run row, source audit entry or failure counter. Such sources remained immediately due and could never reach quarantine. The runner now synthesizes a sanitized, explicit `adapter_error` or `probe_timeout` failure, persists it under the same unexpired per-source lease and revision CAS, applies one-hour retry after the first failure and quarantine after the second. A private timeout sentinel prevents an adapter from forging a timer failure. Adapter-supplied `runnerFailure` fields are rejected, and late abort-listener success cannot override a timeout. D1 failures remain `runner_error` rather than fabricating source failures. Fixture Cron totals expose a separate `probe_failed` count.

**Hardened the server-side rights release gate.** In addition to canonical `rights/YYYY/<source-id>.md` evidence, signing now requires a separate, release-pinned grant with exact source ID, media kind, adapter version, reviewed hostnames, path prefix, review timestamp and expiry. The grant is rejected if it is older than 366 days, future-dated, expired, mismatched, duplicated, malformed or contains unexpected fields. A signed snapshot's TTL cannot outlive the grant. The compiled rights-ref list and grant list are **BOTH EMPTY**. The default Worker refuses to load a signing key or read D1 even for an empty snapshot until both lists have been explicitly reviewed and compiled. Test fixture grants are injected only into offline factory tests; no live source or rights approval was added.

**Isolated verification:** Exact SHA-matched GitHub `policy.mjs` + its committed test passed **18/18**; exact `publication-guard.mjs` + its committed test passed **9/9**. Additional local isolated tests used exact SHA-matched policy, publication guard, runner and Worker source with clearly marked mock dependencies: **55/55** (14 policy, 21 grant, 10 runner, 10 Worker). Total Node **82/82**. The exact committed 0001/0002 SQL migration files passed **8/8** local SQLite crash/lease/CAS/audit regression scenarios using a small Python harness that mirrors the write SQL. These 90 tests do **not** replace the real full Node test suite or actual Worker+D1 integration. The updated committed `runner.test.mjs` and `signed-endpoint.test.mjs` have not yet run with all their exact GitHub dependencies.

**Release blockers unchanged:** Complete Node 22 checkout suite with real SQLite registry, actual isolated Wrangler/workerd D1, real Gradle/.cs3, device beta and explicit human rights approval. No GitHub Actions run, real source probe, live D1/Worker change, production branch change, signing key or rights activation occurred.

## 26 Sep 2026: third offline parkur — parser drift and fail-closed activation

- The runner now snapshots only own, primitive, explicitly allowed adapter fields into a fresh object before any async D1 commit. Unknown fields, getters, symbols, inherited `runnerFailure`, missing episode/search/detail checks, nonboolean values and schema drift become a **single audited structural admin hold**, not a false outage or an infinite retry storm. Actual thrown exceptions/null results remain `adapter_error`; genuine timeout remains `probe_timeout`. Fixture Cron totals include `anomaly_held` separately.
- Production snapshot publication, Access admin reads and admin writes now require `WATCHDOG_FIXTURE_ENABLED === "false"` explicitly. Missing, uppercase or malformed flags are NOT accepted. Admin mutations require an exact raw canonical URL, rejecting empty `?`/`#`, credentials, encoded/normalized path aliases before reading a body.
- Exact GitHub SHA-matched Node adapter contract 6/6 and exact runner source with clearly marked **test-only** fake registry/lease/scheduler 8/8 passed locally. New full exact runner/admin/signed-endpoint tests are committed but **not** yet run as a complete real D1/Worker checkout. No live sources, rights, keys, Actions, deployments or remote D1 writes.

## 26 Sep 2026 — fourth offline parkur: exact access and SQLite verification

- Exact GitHub SHA-matched Cloudflare Access admin-auth module and tests: **9/9 PASS**, including signed RS256 JWT, issuer/audience/expiry, spoofed headers, and hanging JWKS cancel. The 64 KiB JWKS reader no longer awaits an untrusted never-settling `reader.cancel()`.
