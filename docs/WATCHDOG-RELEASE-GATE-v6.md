...
The bridge now returns immediately when the release-compiled rights list is empty, before consulting delivery configuration or restoring any cached snapshot. The source-wiring Node suite contains four checks: reviewed snapshot wiring, empty production registry, empty-rights early return, and rights-before-selection ordering. The new checks are committed but **not yet executed against the exact GitHub checkout**. The release remains disabled for real sources; no live source or remote D1 request was made.
