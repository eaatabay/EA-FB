...
### Evidence path normalization hardening (26 Sep 2026; committed, unrun)
A source's compiled rights evidence reference now rejects empty, `.` and `..` path segments rather than merely checking a broad filename regex and `..`. Four additional offline Kotlin regression cases cover doubled slashes, dot segments, trailing slashes and a canonical valid reference. The rights suite now contains 57 cases. These new cases have not yet been run with the exact current Kotlin checkout; no real sources or live endpoints were enabled.
\n### Additional rights-boundary hardening (26 Sep 2026; committed, unrun)\nThe reviewed-source gate now also rejects a signed URL whose raw authority is not the exact canonical lowercase approved hostname and rejects duplicate IDs in a supplied verified snapshot. These are defense-in-depth checks in addition to the signed-snapshot verifier. Two new Kotlin regressions increase the rights test file to 59 cases. The new regressions have not been executed against the exact GitHub checkout.\n